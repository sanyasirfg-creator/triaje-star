...
SONIDOS_DISPONIBLES = {
    "Ninguno": None,
    "Caos / sirenas": "caos.mp3",
    "Incendio": "incendio.mp3",
    "Explosiones lejanas": "explosiones.mp3",
}


def reproducir_audio_loop(nombre_archivo):
    """Incrusta un audio en loop y autoplay. Requiere interacción previa del
    usuario en la página (por eso va ligado a un checkbox, no autoplay puro)."""
    ruta = os.path.join(CARPETA_SONIDOS, nombre_archivo)
    if not os.path.exists(ruta):
        st.sidebar.warning(f"No se encontró el archivo: {ruta}")
        return
...
# --- UI ---
st.sidebar.header("🔊 Ambiente sonoro")
sonido_elegido = st.sidebar.selectbox("Elige un sonido de fondo", list(SONIDOS_DISPONIBLES.keys()))
if sonido_elegido != "Ninguno":
    st.sidebar.caption("El navegador puede bloquear el audio hasta que interactúes con la página (haz clic en cualquier botón).")
    reproducir_audio_loop(SONIDOS_DISPONIBLES[sonido_elegido])
