...
COSTO_ACCION = 2
UMBRAL_PRECISION = 2  # 2 acciones o menos = bono de precisión
...
UMBRAL_APROBACION_PCT = 0.6  # 60% del puntaje máximo posible
...
# Cada caso describe lo VISIBLE en "nombre" (incluye si camina o no, y el tono
# dramático o silencioso). Los signos vitales reales quedan ocultos hasta que
# el jugador los investiga con los botones de acción.
CASOS = [
...
def clasificar(v):
    """Cascada R -> P -> M, siguiendo el checklist."""
...
def leer_respiracion(v, via_abierta):
    if v["via_aerea_obstruida"] and not via_abierta:
        return 0, True  # lectura falsa: parece que no respira
...
def nueva_ronda():
    ...
    st.session_state.acciones_usadas = 0
    st.session_state.via_abierta = False
    st.session_state.hallazgos = []
    st.session_state.resultado = None


def usar_accion(costo, texto_hallazgo):
    st.session_state.presupuesto -= costo
    st.session_state.acciones_usadas += 1
...
def elegir_color(color_elegido):
    ...
    if color_elegido == color_correcto:
        ...
        if st.session_state.acciones_usadas <= UMBRAL_PRECISION:
    ...
    st.session_state.resultado = {
        ...
        "acciones_usadas": st.session_state.acciones_usadas,
    }
...
if not st.session_state.iniciado:
    ...
    st.write("Investiga cada víctima con los botones de evaluación (cada uno consume tiempo), y clasifícala según R-P-M.")
...
if st.session_state.resultado is None:
    st.markdown("### Acciones de evaluación")
    ...
    with col2:
        if st.button("Contar Respiraciones", use_container_width=True):
            valor, es_falso = leer_respiracion(v, st.session_state.via_abierta)
    ...
    st.write(f"**Acciones usadas:** {r['acciones_usadas']}" + (" 🎯 Bono de precisión aplicado" if r["tipo"] == "correcto" and r["acciones_usadas"] <= UMBRAL_PRECISION else ""))
    if r["penalidad_via"]:
